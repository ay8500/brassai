			...
			if (strcasecmp($fileName[1],"jpg")==0) {
				if ($_FILES['userfile']['size']<2000000) {
					if (move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile)) {
						$picture = array();
						$picture["id"]=-1;
						$picture["classID"]=getRealId(getAktClass());
...
$pictures = $db->getListOfPictures(getRealId(getAktClass()), "class", 2, 2);
			...
			<div style="margin-bottom: 15px;padding: 10px; display: block;border-radius: 10px;background-color: #e8e8e8" >
				<div class="col-sm-8">
					<a style="display: inline-block;vertical-align: top;" title="<?php echo $pict["title"] ?>" href="#" onclick="toggleBigger(this)">
						<img class="img-responsive" style="display: inline-block;" src="convertImg.php?width=1800&thumb=false&id=<?php echo $pict["id"] ?>" />
					</a>
				</div>
...
<?php echo $pict["comment"] ?></textarea>
					...
					<div id="show_<?php echo $pict["id"] ?>" style="display: inline-block;margin: 10px 0px 0 0px; background-color: white;border-radius: 7px;padding: 5px;" >
...
function changeVisibility(id) {
	...
	function unlinkPicture(id) {
		if (confirm("Fénykép végleges törölését kérem konfirmálni!")) {
			window.location.href="tablo.php?action=unlinkPicture&id="+id;
		}
	}
	<?php endif;?>
